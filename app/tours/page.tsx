...
export default function ToursPage() {
  ...
  const [filteredTours, setFilteredTours] = useState<TourPackage[]>(sampleTours)
  ...
  useEffect(() => {
    ...
    if (filters.destinations.length > 0) {
      filtered = filtered.filter((tour) => filters.destinations.includes(tour.destination))
    }

    if (filters.tourTypes.length > 0) {
      filtered = filtered.filter((tour) => filters.tourTypes.includes(tour.category))
    }

    if (filters.accommodationTypes.length > 0) {
      filtered = filtered.filter((tour) => filters.accommodationTypes.includes(tour.accommodationType))
    }

    if (filters.groupSizes.length > 0) {
      filtered = filtered.filter((tour) => filters.groupSizes.includes(tour.groupSize))
    }
    ...
    filtered = filtered.filter((tour) => tour.duration >= filters.duration[0] && tour.duration <= filters.duration[1])
  ...
  return (
    <div className="min-h-screen bg-background safari-texture">
